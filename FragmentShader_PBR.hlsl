...
// Adapted for training by Lari H. Norri for the course "3DCC" at Full Sail University, 2024-2025.
...
struct v2f
{
    ...
    float3 posW : WORLD;
    uint matrixIndex : SV_InstanceID;
};
...
static const int samplerIndex = 0;
...
StructuredBuffer<MATERIAL_DATA> MaterialInfo : register(b2, space0);
...
float4 main(v2f vin) : SV_TARGET
{
    int albedoMap = MaterialInfo[DrawInfo[vin.matrixIndex].materialIndex].baseColorIndex;
    int normalMap = MaterialInfo[DrawInfo[vin.matrixIndex].materialIndex].normalIndex;
    int roughnessMetalOcclusionMap = MaterialInfo[DrawInfo[vin.matrixIndex].materialIndex].metallicRoughnessIndex;
   // Sample input textures to get shading model params.
    float3 albedo = textures[albedoMap].Sample(samplers[samplerIndex], vin.uv).rgb;
    float3 MRA = textures[roughnessMetalOcclusionMap].Sample(samplers[samplerIndex], vin.uv).rgb;
    float3 emissive = textures[emissiveMap].Sample(samplers[samplerIndex], vin.uv).rgb;
    ...
    float3 rawNrm = textures[normalMap].Sample(samplers[samplerIndex], vin.uv).rgb;
    ...
    float3 ambientLighting;
	{
        ...
        float3 irradiance = cubeTextures[irradianceMap].Sample(samplers[samplerIndex], N).rgb;
        ...
        float3 specularIrradiance = cubeTextures[specularMap].SampleLevel(samplers[samplerIndex], Lr, roughness * specularTextureLevels).rgb;

		// Split-sum approximation factors for Cook-Torrance specular BRDF. 
        float2 specularBRDF = textures[brdfMap].Sample(samplers[samplerIndex], float2(cosLo, roughness)).rg;
    ...
    // Combine direct and ambient lighting with emissive term.
    float4 finalColor = float4(directLighting + ambientLighting + emissive, 1.0);
    
    // Recommended: Apply final gamma-curve corrections for linear to SRGB color space.
    // Ideally Albedo and Emissive textures should be loaded in SRGB color space.
    // When you do this, lighing calculations will correctly be done in linear space.
    // However, the final color must be converted back to SRGB for accurate display.
    // finalColor.rgb = pow(finalColor.rgb, 1.0 / 2.2); // inverse gamma curve.
